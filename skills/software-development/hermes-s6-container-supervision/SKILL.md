...
annotation: "s6-overlay: modify, debug, extend container supervision tree"
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
Two approaches, depending on whether you're modifying the upstream Hermes repo
or extending from a downstream Containerfile:

**Upstream (hermes-agent repo `docker/` directory):**

1. Create `docker/s6-rc.d/<name>/type` with `longrun\n` and `docker/s6-rc.d/<name>/run`
2. Drop to hermes via `s6-setuidgid hermes` at the top of run (unless root needed)
3. Create empty `docker/s6-rc.d/<name>/dependencies.d/base`
4. Create empty `docker/s6-rc.d/user/contents.d/<name>`
5. The `COPY docker/s6-rc.d/` in the Dockerfile picks it up automatically

**Downstream (custom Containerfile deriving from the upstream image):**

Use `COPY` + `RUN` to create the service directory and register it in the
user bundle. See `references/downstream-containerfile-services.md` for the
full pattern with worked examples.

**User-context service (daemon bridge that runs as hermes user):**

When the service replaces a cron-launched watchdog and runs as the hermes
user (not root), see `references/user-context-service-pattern.md` for
worked examples covering privilege drop, absolute-path interpreters, and
the differences from system services like cupsd.
...
### Bridge starts but exits immediately (converted cron watchdog)

When migrating a cron-watchdog-launched daemon to s6, the original watchdog
script typically used `nohup ... &` to background the process and then
periodically checked health. s6 `run` scripts must **not** background the
process — use `exec <command>` (no nohup, no &, no PID file). Without
`exec`, a shell stays resident as the supervised PID and s6 tracks the
wrong process. See `references/user-context-service-pattern.md`.

### Bridge command not found or wrong python/node

s6 `run` scripts use `#!/command/with-contenv sh` which loads
`/run/s6/container_environment/` — NOT the hermes user's shell profile.
Use **absolute paths** for interpreters:
- `/opt/hermes/.venv/bin/python3` not `python3`
- `/usr/bin/node` not `node`
The hermes venv is at `/opt/hermes/.venv` — do not rely on PATH.
